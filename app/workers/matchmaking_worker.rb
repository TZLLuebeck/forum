class MatchmakingWorker
  ...
  def perform(id, ownerid)
    ...
    sql = 'SELECT interests.id, interests.title, interests.user_id, users.email, users.news FROM (SELECT interests.*, tags.name, COUNT(tags.id) AS count FROM interests INNER JOIN ('+
